...
==================== v3 优化播放速度 ====================
1. 增加播放地址缓存(缓存10分钟), 同一集重复播放直接返回, 避免重复请求
2. 减少重试次数(2→1), 降低超时(15s→10s), 加快请求失败判定
3. 调整重试间隔(1.2s→0.8s)

==================== v4 进一步加速片源加载 ====================
1. 播放页请求仅读取前8KB(只提取player_aaaa即可), 大幅减少传输耗时
2. 播放页专用超时缩短为6秒(普通页面保持10秒)
3. 增加连接超时参数, 快速失败避免卡死
4. 优化正则搜索, 减少冗余匹配
5. 若提取到的video_url是解析链接(parse=1), 直接返回, 不再额外处理
=========================================================================
"""
...
class Spider(Spider):
    ...
    def __init__(self):
        ...
        # 播放地址缓存 {cache_key: {'data': result, 'expire': timestamp}}
        self._cache = {}
    ...
    # 网络请求(带重试及部分读取优化)
    # - retries: 重试次数
    # - max_read: 仅读取前N字节(用于播放页快速获取player_aaaa), 0表示全部读取
    # - timeout: 可单独指定超时(播放页用6秒, 普通用10秒)
    # ------------------------------------------------------------------
    def _get_html(self, url, retries=1, max_read=0, timeout=10):
        full_url = urljoin(self.HOST, url) if not str(url).startswith('http') else url
        last = ''
        for i in range(retries + 1):
            try:
                # 使用stream模式，以便限制读取量
                with self.session.get(full_url, headers=self.headers, timeout=(3, timeout), stream=True) as resp:
                    if resp.status_code == 200:
                        if max_read > 0:
                            # 只读取前max_read字节，然后关闭连接
                            content = b''
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        cache_key = id
        now = time.time()
        if cache_key in self._cache and self._cache[cache_key]['expire'] > now:
            cached = self._cache[cache_key]['data']
            print(f'[播放缓存命中] {id}')
            return cached

        play_url = urljoin(self.HOST, id) if str(id).startswith('/') else str(id)
        # 播放页只读取前8KB（足够获取 player_aaaa），超时6秒
        html = self._get_html(play_url, retries=1, max_read=8192, timeout=6)
        result = {"parse": 1, "url": play_url, "header": self.headers}
        ...
        if html:
            ...
            if video_url:
                ...
                if self._is_direct_media(video_url):
                    ...
                    play_headers['Accept'] = '*/*'
                    result = {"parse": 0, "url": video_url, "header": play_headers}
                else:
                    # 解析链接直接交给播放器嗅探，无需额外操作
                    result = {"parse": 1, "url": video_url, "header": play_headers}
            # 若提取失败，保留 parse=1 让播放器自行处理
...
if __name__ == '__main__':
    ...
    def self_test():
        ...
        for name, fix, want_parse, want_frag in cases:
            spider._get_html = lambda u, r=2, m=0, t=10, f=fix: f
