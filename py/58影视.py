...
class Spider(Spider):
    ...
    # 2.7.10 起，源站的新游客注册增加了浏览器指纹挑战和 PoW 校验。
    # 保留一套已验证、仅具游客权限的凭据作为冷启动兜底，避免 OK影视清缓存、
    # 重装或更换模拟器后再次走旧注册接口并卡在“设备验证信息缺失”。
    DEFAULT_AUTH_UUID = "adad75b0-fb53-4116-a951-393dd1b2cf1d"
    DEFAULT_AUTH_SECRET = "bddb9b833032e5b1203c9af0561a563da448f085c2c059070f96af139406495c"
    ...
    def init(self, extend):
        ...
        if not self.secret and self.DEFAULT_AUTH_UUID and self.DEFAULT_AUTH_SECRET:
            self.device_id = self.DEFAULT_AUTH_UUID
            self.secret = self.DEFAULT_AUTH_SECRET
        self._reset_session()
        if self.secret:
