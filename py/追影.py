class Spider(Spider):
    ...
    # ====================== 修复后的 categoryContent ======================
    def categoryContent(self, tid, pg, filter, extend):
        pg = str(pg)  # 关键修复：确保页码为字符串
        result = {}
        try:
            ...
            # 修复：从页面提取真实总页数
            total_page = 1
            page_match = re.search(r'共<strong>(\d+)</strong>页', html)
            if page_match:
                total_page = int(page_match.group(1))
            # 若未找到，尝试其他常见格式
            if total_page == 1:
                page_match2 = re.search(r'pagecount["\']?\s*[:=]\s*(\d+)', html)
                if page_match2:
                    total_page = int(page_match2.group(1))

            result['list'] = videos
            result['page'] = pg
            result['pagecount'] = total_page
            result['limit'] = 30
            result['total'] = total_page * 30
        except Exception as e:
