# 蜜果-芒果TV兼容版 (TVbox电视修复版)
# by @6666 & 社区修复
# 修复电视端 parse=1 拼接失败问题，改为 parse=0 直出完整链接
import sys
import time
import json
import hashlib
...
class Spider(Spider):
    ...
    def homeVideoContent(self):
        data = self.fetch(
            f'{self.mhost}/dynamic/v1/channel/index/0/0/0/1000000/0/0/17/1354?type=17&version=5.0&t={str(int(time.time()*1000))}&_support=10000000',
        ...
        for i in data.get('data', []):
            if i.get('DSLList'):
                for j in i['DSLList']:
                    if j.get('data', {}).get('items'):
                        for k in j['data']['items']:
                            videoList.append({
                                'vod_id': k.get("videoId", ""),
                                'vod_name': k.get('videoName', ''),
                                'vod_pic': k.get('img', ''),
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        body.update(extend)
        data = self.fetch(
            f'{self.host}/rider/list/pcweb/v3',
            params=body,
            headers=self.headers
        ).json()
        videoList = []
    ...
    def detailContent(self, ids):
        ...
        pagecount = data.get('data', {}).get('total_page', 1) or 1
        if int(pagecount) > 1:
    ...
    def searchContent(self, key, quick, pg="1"):
        data = self.fetch(
            f'{self.shost}/applet/search/v1?channelCode=mobile-wxap&q={key}&pn={pg}&pc=10&_support=10000000',
            headers=self.headers
        ).json()
        videoList = []
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if str(id).startswith('mgtvlive@'):
            ...
            try:
                ...
                print(f"Live source error: {e}")
            # 降级：尝试使用备用静态直播流（示例，实际可能无效）
            return {
                'jx': 0,
                'parse': 0,
                'url': '',  # 可替换为已知有效的备用直播地址
        ...
        # ----- 普通视频（电视版修复：直接返回完整解析链接，parse=0） -----
        play_url = str(id)
        if play_url.startswith('/'):
            play_url = f'{self.rhost}{play_url}'

        # 使用与手机版相同的解析接口（若失效可替换）
        parser_url = 'https://jx.aidouer.net/?url='
        # 直接拼接完整地址，避免TVbox内部拼接错误
        full_url = parser_url + play_url

        # 强制使用手机版UA，确保解析站返回移动端兼容流
        mobile_ua = 'Mozilla/5.0 (Linux; Android 13; SM-G998B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Mobile Safari/537.36'
        return {
            'jx': 0,
            'parse': 0,          # 关键修复：改为0，让播放器直接请求完整链接
            'url': full_url,
            'header': {
    ...
    def getf(self, body):
        ...
        data = self.fetch(
            f'{self.host}/rider/config/channel/v1',
            params=params,
            headers=self.headers
        ).json()
        ft = []
