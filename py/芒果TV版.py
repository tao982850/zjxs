...
class Spider(Spider):
    ...
    def _extract_vid(self, url):
        """从播放页URL中提取视频ID（vid），支持多种格式"""
        if not url:
            return None
        # 尝试匹配 /b/xxx 或 /b/xxx? 或 /b/xxx/
    ...
    def _get_mgtv_play_url(self, vid):
        """通过官方接口获取播放地址，含多个尝试方案"""
        # 方案1：使用 /player/getSource（标准接口）
        params = {
            'vid': vid,
            'type': 'b',
            'src': 'mgtv',
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if info.get('url'):
                    return info['url']
                print(f'官方接口返回无流地址，完整响应: {data}')
            else:
                print(f'官方接口状态码异常: {resp.status_code}')
        except Exception as e:
            print(f'官方播放源获取失败(方案1): {e}')

        # 方案2：使用 /video/source 接口（备用）
        try:
            params2 = {
                'vid': vid,
                'type': 'b',
            ...
            if resp2.status_code == 200:
                data2 = resp2.json()
                stream2 = data2.get('data', {}).get('stream', [])
                if stream2:
                    return stream2[-1].get('url')
        except Exception as e:
            print(f'官方播放源获取失败(方案2): {e}')
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        backup_parsers = [
            ...
            'https://jx.xmflv.com/?url=',  # 原解析，可能恢复
        ]
        # 返回第一个解析（若希望自动轮换，可在此处循环尝试，但会耗时）
        return {
            'jx': 0,
            'parse': 1,
            'playUrl': backup_parsers[0],
            'url': full_url,
            'header': {
                'User-Agent': self.headers['User-Agent'],
                'Referer': f'{self.rhost}/'
