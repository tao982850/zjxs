...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        data = self.post(
            ...
            json=self.body, headers=self.headers, verify=True).json()
        ndata = data['data']
    ...
    def newChannelContent(self, tid, pg):
        ...
        try:
            ...
            data = self.post(
                ...
                json=body, headers=self.headers, verify=True
            ).json().get('data', {})
        except Exception as e:
            # [安全修复] 移除可能泄露敏感信息的打印，改用安全日志（此处仅简单print，生产环境建议用logging）
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        data = self.post(f'{self.apihost}/trpc.videosearch.mobile_search.MultiTerminalSearch/MbSearch?vplatform=2',
                         json=body, headers=self.headers, verify=True).json()
        vlist = []
    ...
    def gethtml(self, url, verify=True):
        rsp = self.fetch(url, headers=self.headers, verify=verify)
    ...
    def get_filter_data(self, cid):
        ...
        data = self.post(
            f'{self.apihost}/trpc.universal_backend_service.page_server_rpc.PageServer/GetPageData?video_appid=1000005&vplatform=2&vversion_name=8.9.10&new_mark_label_enabled=1',
            json=hbody, headers=self.headers, verify=True).json()
        return cid, data
