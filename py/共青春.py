...
class Spider(Spider):
    ...
    @staticmethod
    def _try_b64(u):
        """maccms encrypt=2 时 url 为 base64"""
        if re.fullmatch(r'[A-Za-z0-9+/=\s]+', u or ''):
            pad = '=' * (-len(u) % 4)
            try:
                dec = base64.b64decode(u + pad).decode('utf-8', 'ignore')
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        # 先取可能是地址的那一项
        if b and (b.startswith("http") or "/bofang/" in b):
            url = b
        elif a and (a.startswith("http") or "/bofang/" in a):
            url = a
        elif b.startswith("/"):
            url = urljoin(HOST, b)
        elif a.startswith("/"):
        ...
        if not url:
            return {"url": ""}
        try:
            r = self.fetch(url, headers={"User-Agent": UA, "Referer": HOST + "/"}, timeout=30000)
            h = r.text if hasattr(r, 'text') else str(r)
        except:
            return {"url": ""}

        # 1) maccms 标准: var player_aaaa={...}
        m = re.search(r'player_aaaa\s*=\s*(\{[\s\S]*?\})\s*;', h)
        if m:
            try:
                cfg = json.loads(m.group(1))
                u = self._js_unescape(str(cfg.get('url', ''))).strip()
        ...
        # 2) 其它 JS 配置块里的 url 字段 (含转义/编码)
        for jm in re.finditer(r'"url"\s*:\s*"([^"]+)"', h):
            u = self._js_unescape(jm.group(1)).strip()
            if u.startswith('http') and ('.m3u8' in u or '.mp4' in u):
                return {"url": u}
            dec = self._try_b64(u)
            if dec and ('.m3u8' in dec or '.mp4' in dec):
                return {"url": dec}

        # 3) 整页反转义后找 m3u8 / mp4 直链
        h2 = h.replace('\/', '/')
        dm = re.search(r'"dmId"\s*:\s*"([^"]+)"', h2)
        if dm:
            u = dm.group(1)
            if u.startswith('http'):
                return {"url": u}
        m3u8 = re.search(r'(https?://[^\s"\'<>]+\.(?:m3u8|mp4)(?:\?[^\s"\'<>]*)?)', h2)
        if m3u8:
