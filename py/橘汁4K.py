...
class Spider(BaseSpider):
    ...
    # ---------- 缓存变量（首页 & 播放线路） ----------
    _home_cache = None          # 缓存 homeContent 的分类结果
    _home_cache_time = 0
    _play_cache = {}            # 缓存 playerContent 的结果，键为 id（token）
    ...
    def _post(self, path, body):
        ...
        # 超时从 18s 缩短至 10s
        r = self.session.post(self.host + path, data=body, headers=self._public_headers(True), timeout=10)
        r.raise_for_status()
        return r.content
    ...
    def _get_json(self, path):
        if not self.session or not self.host:
            raise RuntimeError("橘汁接口域名获取失败")
        # 超时从 18s 缩短至 10s
        r = self.session.get(self.host + path, headers=self._public_headers(False), timeout=10)
        r.raise_for_status()
        return r.json()
    ...
    # ---------- 优化后的 homeContent（增加缓存，按需加载筛选） ----------
    def homeContent(self, filter):
        # 如果 filter=False 且缓存有效，直接返回缓存的分类（不含筛选）
        if not filter and self._home_cache is not None:
            if time.time() - self._home_cache_time < self._cache_ttl:
                cached = self._home_cache
                return {"class": cached.get("class", []), "filters": {}}
        ...
        try:
            ...
            for item in obj.get("data") or []:
                ...
                tid = str(item.get("id", ""))
                if tid:
                    classes.append({"type_id": tid, "type_name": str(item.get("name", ""))})

            if not filter:
                self._home_cache = {"class": classes}
            ...
            self._home_cache = {"class": classes, "filters": filters}
            self._home_cache_time = time.time()
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        try:
            ...
            for entry in f.get(6, []):
                ...
                if k:
                    headers[k] = v
            url = self._text(self._first(f, 1))
            result = {"parse": 0, "url": url, "header": headers}
            # 存入缓存
            self._play_cache[cache_key] = {"time": now, "result": result}
            return result
        except Exception as e:
