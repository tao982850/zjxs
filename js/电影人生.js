...
// ★★★ 请从浏览器复制 Cookie 填入下面的变量 ★★★
// 打开网站首页，按 F12 -> 应用程序/存储 -> Cookie，复制全部 Cookie 字符串
const COOKIE = ""; // 例如 "sion_id=6a7751444b7bd5c9c6442db3; other=value"
...
async function fetchAndParse(url){
  ...
  try{
    let resp=await req(url,{method:"GET",headers:getHeaders()});
...
async function home(filter){
  let list=[];
  try{
    let html=(await req(appConfig.siteUrl,{method:"GET",headers:getHeaders()})).content||"";
...
// ========== category（保留方案一，仅分页，不带过滤参数） ==========
async function category(tid, pg, filter, extend) {
  pg = pg || 1;
  extend = extend || {};
  let cat = classList.find(c => c.type_id === tid);
  if (!cat) return JSON.stringify({ list: [], pagecount: 0, page: pg, limit: 20, total: 0 });

  let baseUrl = cat.url;
  let url = baseUrl;
  if (pg > 1) {
    url += (baseUrl.includes('?') ? '&' : '?') + 'page=' + pg;
  }
  let fullUrl = appConfig.siteUrl + url;

  let result = await fetchAndParse(fullUrl);
  if (result.list.length === 0) {
    console.warn("[电影人生] 分类无数据，回退首页");
    let h = await fetchAndParse(appConfig.siteUrl);
    result.list = h.list.slice(0, 30);
    result.pagecount = 1;
  }
  result.page = pg;
  result.limit = 20;
...
async function detail(id){
  try{
    let html=(await req(appConfig.siteUrl+id,{method:"GET",headers:getHeaders()})).content||"";
...
async function play(flag,id,flags){
  try{
    if(id.startsWith("http"))return JSON.stringify({parse:0,Header:getHeaders(),url:id});
    let html=(await req(appConfig.siteUrl+id,{method:"GET",headers:getHeaders()})).content||"";
    // player_aaaa解密
    let ps=html.match(/var\s+player_aaaa\s*=\s*(\{[\s\S]+?\})\s*<\/script>/);
    if(ps){try{let d=JSON.parse(ps[1]);let enc=d.encrypt||0,u=d.url||"";if(u){if(enc===2)u=atob(u);if(enc===1)u=decodeURIComponent(u);if(u.startsWith('http'))return JSON.stringify({parse:0,Header:getHeaders(),url:u});}}catch(e){}}
    // /api/m3u8
    let m3=html.match(/href="\/api\/m3u8\?origin=([^&]+)&amp;?url=([^"]+)"/)||html.match(/href="\/api\/m3u8\?origin=([^&]+)&url=([^"]+)"/);
    if(m3){let u=appConfig.siteUrl+"/api/m3u8?origin="+encodeURIComponent(m3[1])+"&url="+m3[2];return JSON.stringify({parse:0,Header:getHeaders(),url:u});}
    // 直链
    let um=html.match(/"url"\s*[:=]\s*"([^"]+\.(m3u8|mp4|flv)[^"]*)"/);
    if(um)return JSON.stringify({parse:0,Header:getHeaders(),url:um[1].replace(/\\/g,'')});
    // iframe
    const $=cheerio.load(html);let ifs=$("iframe").attr("src");if(ifs)return JSON.stringify({parse:1,Header:getHeaders(),url:fixUrl(ifs)});
    let vs=$("video").attr("src");if(vs)return JSON.stringify({parse:0,Header:getHeaders(),url:fixUrl(vs)});
    return JSON.stringify({parse:1,Header:getHeaders(),url:appConfig.siteUrl+id});
