...
function detailParse(detailObj) {
    ...
    if (p === "*") {
        ...
        playFrom = playFrom.map(t => t.replace(/^[\u4e00-\u9fa5]+IY[—\-–—]*/, (m)=>{let s=atob('RWFzeVRW');return s;}));
...
function vodDeal(vod) {
    ...
    vod.vod_play_from = tab_list.join("$$$").split("$$$").map(t => t.replace(/^[\u4e00-\u9fa5]+IY[—\-–—]*/, (m)=>{let s=atob('RWFzeVRW');return s;})).join("$$$");
