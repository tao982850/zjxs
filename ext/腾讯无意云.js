...
var rule = {
    ...
    play_parse: true,
    lazy: $js.toString(() => {
        // 腾讯链接最终是网页地址，需要交给 TVBox 的解析器处理。
        // 不要设置 parse:0，否则 TVBox 会当成直链去播放，导致黑屏。
    ...
    一级: $js.toString(() => {
        ...
        if (fyclass === 'mini_series') {
            ...
            $.forEach(function(it) {
                ...
                // data-float 往往是 JSON 或者带参数的串，需要提取纯 cid
                let cid = '';
                try {
                    let obj = JSON.parse(item);
                    cid = obj.cid || obj.vid || '';
    ...
    二级: $js.toString(() => {
        ...
        // 严谨提取 sourceId / cid，避免 split 带出多余参数
        let sourceId = '';
