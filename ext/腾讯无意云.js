...
//  >>> 修复：filter_url 补全基础参数 / 一级 JSON 回退 / 二级安全解析 cid <<<
...
var rule = {
    ...
    filter_url: '_all=1&append=1&channel=fyclass&listpage=1&offset=((fypage-1)*21)&pagesize=21&sort={{fl.sort or 75}}&itype={{fl.itype}}&ipay={{fl.ipay}}&iarea={{fl.iarea}}&iyear={{fl.iyear}}&theater={{fl.theater}}&award={{fl.award}}&recommend={{fl.recommend}}&recommend_1={{fl.recommend_1}}&recommend_2={{fl.recommend_2}}&recommend_3={{fl.recommend_3}}&producer={{fl.producer}}&characteristic={{fl.characteristic}}&exclusive={{fl.exclusive}}&itrailer={{fl.itrailer}}&iregion={{fl.iregion}}&pay={{fl.pay}}&anime_status={{fl.anime_status}}&item={{fl.item}}&all={{fl.all}}&gender={{fl.gender}}&language={{fl.language}}&child_ip={{fl.child_ip}}&prefer={{fl.prefer}}&story={{fl.story}}&identity={{fl.identity}}&attraction={{fl.attraction}}',
    ...
    一级: $js.toString(() => {
        ...
        if (fyclass === 'mini_series') {
            ...
            /* ========== 非短剧：先试 JSON，再回退 HTML ========== */
            let html = fetch(input, fetch_params);
            log('一级URL: ' + input);
            log('一级返回前 500: ' + html.substring(0, 500));

            let parsedJson = null;
            try { parsedJson = JSON.parse(html); } catch (e) { parsedJson = null; }

            if (parsedJson && parsedJson.data && parsedJson.data.list) {
                parsedJson.data.list.forEach(function(it) {
                    let img = it.pic || it.image_url || '';
                    if (img && img.startsWith('//')) img = 'https:' + img;
                    let cid = it.cid || it.id || '';
                    if (!cid) return;
                    d.push({
                ...
                $.forEach(function(it) {
                    let floatStr = pdfh(it, 'a&&data-float') || '';
                    ...
                    let cid = '';
                    try {
                        let o = JSON.parse(floatStr);
                        cid = o.cid || '';
                    } catch (e) {
                        let m = floatStr.match(/cid["=:]?\s*["]?([A-Za-z0-9]+)/);
                        cid = m ? m[1] : '';
                    }
    ...
    二级: $js.toString(() => {
        ...
        if (/get_playsource/.test(input)) {
            let m = input.match(/id=([A-Za-z0-9]+)/);
            sourceId = m ? m[1] : '';
        } else {
            let m = input.match(/\/x\/cover\/([A-Za-z0-9]+)/);
            if (!m) m = input.match(/cid=([A-Za-z0-9]+)/);
            sourceId = m ? m[1] : '';
        }
        let cid = sourceId;
        if (!cid) {
            log('二级无法解析 cid, input=' + input);
            return;
        }
        ...
        if (/get_playsource/.test(input)) {
            ...
            if (json && json.c && json.c.video_ids) {
                    ...
                    if (it.doc && it.doc.id && it.videoInfo &&
                        ...
                        if (!seenIds.has(itemId)) {
                            ...
                            d.push({
                                ...
                                img: imgUrl,
                                url: 'https://v.qq.com/x/cover/' + itemId + '.html',
